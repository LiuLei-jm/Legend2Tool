...
{
    /// <summary>
    /// 可替换的脚本优化服务实现。当前阶段保留原有处理逻辑，后续按功能组件逐步迁移。
    /// </summary>
    public class ModularScriptOptimizationService : IScriptOptimizationService
    {
        ...
        public ModularScriptOptimizationService(
        ...
        {
            var result = _callReferenceParser.Parse(line);
            callPath = result.Path;
            callField = result.Field;
        }
        ...
        public async Task OptimizingMinMonBurstRateAsync(int minMonBurstRate)
        {
            ...
            {
                totalFiles += callBurstRateFilePaths.Count;
            ...
            _progressStore.ProgressText = "优化完成.";
            progress.Report(_progressStore);
        }
